...
### 2.1. Phong cách chủ đạo: Soft Glass-Card UI (Hybrid)

Hệ thống HEMIS sử dụng phong cách thiết kế **Soft Glass-Card UI (Hybrid)** - hay còn gọi là **Modern SaaS Minimalism**. Đây là sự kết hợp giữa tính thực dụng của cấu trúc Card-based và nét hiện đại của Glassmorphism, giúp tối ưu hóa không gian hiển thị và giảm tải áp lực thị giác cho người dùng.

**Cập nhật thêm:**

- **Backdrop Blur:** `backdrop-blur-md` (12px).
- **Surface Opacity:** `bg-white/90` cho các khối nội dung chính.
- **Shadow Code:** `shadow-[0_8px_30px_rgb(0,0,0,0.04)]` (Đổ bóng mềm lớp ngoài).
- **Border Code:** `border border-white/20` hoặc `border-outline-variant`.

| **STT** | **Đặc điểm chính**             | **Mô tả phong cách chi tiết**                                       | **Ứng dụng trong HEMIS**                                                                 |
| ------- | ------------------------------ | ------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| 1       | **Layering (Phân lớp)**        | Sử dụng các lớp chồng lên nhau thay vì đường kẻ (border) cứng nhắc. | Card trắng (`#FFFFFF`) chứa dữ liệu nằm trên nền xám nhạt (`#F4F3F8`).                   |
| 2       | **Soft Shadows (Đổ bóng mềm)** | Đổ bóng với dải mờ (blur) lớn, độ đậm thấp (< 10%).                 | Tạo cảm giác các khối Payload dữ liệu đang "bay" nhẹ trên bề mặt, tạo chiều sâu.         |
| 3       | **High Border Radius**         | Các góc được bo tròn lớn (từ 8px - 16px).                           | Làm mềm các bảng dữ liệu (Table) và các ô nhập liệu (Input), tạo cảm giác thân thiện.    |
| 4       | **Subtle Transparency**        | Độ trong suốt nhẹ (Opacity 90-95%) kèm hiệu ứng Backdrop Blur.      | Áp dụng cho Header và Sidebar để người dùng cảm nhận được không gian hệ thống.           |
| 5       | **Clean Typography**           | Font Sans-serif hiện đại, khoảng cách dòng (line-height) thoáng.    | Tăng khả năng đọc (Readability) cho các tài liệu kỹ thuật và bảng số liệu phức tạp.      |
| 6       | **Accent Color Focus**         | Tổng thể trung tính, chỉ dùng màu thương hiệu cho Action.           | Sử dụng **Primary Blue (#094874)** duy nhất cho nút bấm chính, liên kết và icon chỉ thị. |

### 2.2. Phân cấp thành phần (Atomic Design Hierarchy)
...
- **Kích thước Logo:** Cố định `w-16` (64px).
- **Typography Brand:** - **HEMIS:** `text-2xl`, `font-bold`, màu `#094874`.
    - **SYSTEM:** `text-lg`, `font-light`, màu `#094874`.
...
- **Cấu trúc cô lập (Isolated Scroll):** - `TableBoard` phải có `overflow-auto` và giới hạn chiều cao `max-h-[calc(100vh-400px)]` để khóa thanh cuộn nội bộ.
- **Cơ chế Sticky (Z-Index Hierarchy):**
    - **Cấp 1 (Z-100):** `HeaderContainer` (Luôn nằm trên bảng).
    - **Cấp 2 (Z-50):** Các cột cố định (`Checkbox`, `STT`, `Mã ngành`) và giao điểm tiêu đề.
    - **Cấp 3 (Z-20):** Tiêu đề bảng (`thead th`) khi cuộn dọc.
- **Định dạng ô:** `whitespace-nowrap` để bảo toàn tính toàn vẹn của dữ liệu chuyên ngành.
---
...
1. **Netflix Tech Blog:** [Practical Design Systems](https://netflixtechblog.com/) - Cách họ phân lớp giao diện quản trị.
2. **Microsoft Design:** [Fluent Design System - Depth & Elevation](https://www.microsoft.com/design/fluent/) - Quy chuẩn về đổ bóng mềm và phân lớp.
3. **Vercel Design:** [Geist UI](https://geist-ui.dev/) - Tiêu chuẩn về Modern SaaS Minimalism.
